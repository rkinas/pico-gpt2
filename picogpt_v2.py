...
def O(s):
 ...
 s.g=1
 for n in t[::-1]:
  for c,l in n.p:c.g+=l*n.g
Q=range
T=sum
I=len
E=16
A=lambda x,y:[a+b for a,b in zip(x,y)]
S=[[[V(r.gauss(0,.08))for _ in Q(c)]for _ in Q(w)]for w,c in zip([Z,E,Z,E,E,E,E,4*E,E],[E]*8+[4*E])]
P=T(T(S,[]),[])
M=[0.]*I(P)
N=M[:]
F=lambda x,w:[T(a*b for a,b in zip(o,x))for o in w]
J=lambda x:(c:=(T(a*a for a in x)*I(x)**-1+1e-5)**-.5,[a*c for a in x])[1]
X=lambda g:(e:=[V((z:=m.exp((a:=v+-max(c.d for c in g)).d)),((a,z),))for v in g],t:=T(e),[i*t**-1 for i in e])[2]
def G(i,p,K,C):
 x=J(A(S[0][i],S[1][p]))
 x=J(y:=x)
 ...
 for h in Q(4):
  e=h*4
  u=q[e:e+4]
  w=X([T(u[j]*K[t][e+j]for j in Q(4))*.5 for t in Q(I(K))])
...
for s in Q(1000):
 ...
 for p in P:
  M[i]=.85*M[i]+.15*p.g
  N[i]=.99*N[i]+.01*p.g**2
  p.d-=z*(M[i]/(1-.85**j))/((N[i]/(1-.99**j))**.5+1e-8)
  p.g=0
  i+=1
 print(s+1,L.d)
for _ in Q(20):

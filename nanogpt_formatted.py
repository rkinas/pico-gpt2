...
r.seed(42)
os.path.exists("input.txt") or __import__("urllib.request").request.urlretrieve(
...
V = type(
    ...
    (),
    {
        ...
        "__add__": lambda s, o: V(
            s.d + (o := o if type(o) == V else V(o)).d, (s, o), (1, 1)
        ),
        "__mul__": lambda s, o: V(
            s.d * (o := o if type(o) == V else V(o)).d, (s, o), (o.d, s.d)
        ),
...
X = lambda g: (lambda e: (lambda t: [i / t for i in e])(sum(e)))(
    [ex(v + -max(a.d for a in g)) for v in g]
)
J = lambda x: (lambda c: [a * c for a in x])(
    (sum(a * a for a in x) / len(x) + 1e-5) ** -0.5
)
        ...
        [
            (
                lambda q, k, v: (
                    ...
                    X(
                        [
                            sum(q[j] * k[t][j] for j in range(R)) / R**0.5
                ...
                q[h * R : (h + 1) * R],
                [i[h * R : (h + 1) * R] for i in K[0]],
                [i[h * R : (h + 1) * R] for i in C[0]],
            )
            for h in range(H)
        ],
        [],
...
g = lambda K, C, t, p, s: (
    ...
    or (
        n := r.choices(
            range(Z), weights=[a.d for a in X([l * 2 for l in G(t, p, K, C)])]
        )[0]
    )
    == B
